reviewcount_barplot <- function(df, xaxis_size = 6, xaxis_rows = 2){
    # data preparation
    df_plot <- df %>%
        group_by(country) %>%
...
# create orders for consistent bar order - use  the q1 plotorder function
    ...
    g <- df_plot %>%
        ggplot() +
        geom_bar(aes(x = country, y = Count_reviews, fill = country), stat = "identity", show.legend = FALSE) +
        geom_text(aes(x = country, y = Count_reviews, label = round(Median_score, 1)), vjust = -0.3, size = 1.5) +
        theme_bw() +
       ...
       theme(plot.title = element_text(size = 14),
             ...
             axis.text.x = element_text(size = xaxis_size, angle = 90, hjust = 1, vjust = 0.5))
